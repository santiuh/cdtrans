<template>
  <div
    class="flex flex-row min-h-screen font-sans !text-black w-full lg:max-w-[1440px] px-10 place-self-center"
  >
    <aside
      class="w-fit lg:w-64 bg-white p-5 border-gray-200 lg:min-h-screen shadow-[4px_0_6px_-1px_rgba(0,0,0,0.1),2px_0_4px_-1px_rgba(0,0,0,0.06)] flex-shrink-0"
    >
    ...
    <main class="flex-1 p-6 lg:p-10 bg-gray-50 overflow-y-auto">
      <div v-if="selectedProduct" class="bg-white rounded-lg shadow p-6 lg:p-8">
        <button
        ...
        <div class="flex flex-col lg:flex-row gap-6 lg:gap-8">
          <div class="lg:w-1/2 flex-shrink-0">
            <div
              class="w-full h-64 lg:h-80 bg-gray-200 rounded-md overflow-hidden flex items-center justify-center text-gray-500 border border-gray-200"
            >
              <img
                :src="
                  selectedProduct.imagen ||
                  `https://placehold.co/600x400/f87171/ffffff?text=${encodeURIComponent(
                    selectedProduct.nombre
                  )}`
                "
      ...
      <div v-else>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <p v-else class="text-gray-500 col-span-full">
            {{
              selectedCategory
